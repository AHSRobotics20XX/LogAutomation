...
@TeleOp(name="LogExample", group="Linear Opmode")
@Disabled
public class LogExample extends LinearOpMode {
    ...
    public void runOpMode()
    {
        ...
        while (opModeIsActive())
        {
            ...
            //Setting the Pwr variables using the controller(s)
            ...
            //When trying to log there has to be some form of toggle so you don't have to hold down the buttons all the time
            //This uses gamepad1 bumpers and buttons to turn on boolean variables that are inside of the LogClass
            //NOTE: To turn off logging, stop the program.
                //>>>>>>>>>After each record attempt, please clear the logcat from the phone<<<<<<<<<
            ...
            //This calls the LogClass method of logging the devices and detection
            ...
            {
                //To start searching in ObjectDetection you need to specify the camera
                //Then using the UseObjectDetectionClass call the method UseVuforiaAndTensorFlow()
                hardwareDeviceClass.GetCamera(camera).UseObjectDetectionClass().UseVuforiaAndTensorFlow();

                //This logs the found label from detection
                logClass.LogObjectDetection();
            }

            //This allows telemetry to be seen on DS when log mode is on and, if using object detection, what it found
